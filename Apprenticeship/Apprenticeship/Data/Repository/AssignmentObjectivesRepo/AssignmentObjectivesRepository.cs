...
namespace Apprenticeship.Data.Repository.AssignmentObjectivesRepo
{
    public class AssignmentObjectivesRepository : IAssignmentObjectivesRepository
    {
        ...
        {
            return context.assignmentObjectives.Include(tc => tc.objective).Include(tc => tc.assignment).ToList();
        }

        public AssignmentObjectives GetAssignmentObjective(int objectiveId, int assignmentId)
        {
            var assignmentObjectives_ = context.assignmentObjectives.Include(tc => tc.objective).Include(tc => tc.assignment).Where(tc => tc.objectiveId == objectiveId && tc.assignmentId == assignmentId).SingleOrDefault();
